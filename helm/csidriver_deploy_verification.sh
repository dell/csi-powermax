...
nodescount=$(kubectl get nodes -o wide --namespace $namespacevar | awk ' /Ready/{ print $2; }'| wc -l)
         if [ $nodescount -gt $n ]
         then
                echo 'Nodes are  succesfully deployed in the  cluster.'
         else
...
echo "########### PODS AND ITS RUNNING STATUS #############"
max=300
totaltimespent=0
i=10
podscount=$(kubectl get pods -o wide --namespace $namespacevar | grep -v NAME | awk '{print $1;}'| wc -l)
        ...
        then
                ...
                if [ $totaltimespent = $max ]
                then
                        echo  "All pods are not running in the cluster"
                        exit 1
                else
                        echo  "All the pods are running in cluser "
                fi
        else
                echo "Deployment of pods in the cluster is unsuccessfull"
                  exit 1
        fi
...
storageclassnum=$(kubectl get storageclass --namespace  $namespacevar | grep -v NAME| wc -l)
        ...
        then
              echo 'Storage class has been  succesfully Created.'
        else
          ...
          availablenodes=$(kubectl get daemonset  --namespace  $namespacevar |grep -v NAME |  awk '{ print $6; }')
                ...
                then
                    if [ $desirednodes = $availablenodes ]; then
                      echo "daemonset service  are running, deamon service name desirednodes availablenodes  ::   " $daemonsetnodename $desirednodes $availablenodes
                    else
                      echo " All the available nodes of daemonset service are not running"
                      exit 1
                    fi
                else
